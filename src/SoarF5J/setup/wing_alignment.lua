---------------------------------------------------------------------------
-- SoarF5J flaps and aileron alignment, loadable component               --
--                                                                       --
...
local function safeFieldId(name)
  local field = getFieldInfo(name)
  if not field or not field.id then
    setError("Missing field " .. name .. ". Check model inputs.")
    return nil
  end
  return field.id
end
...
local function drawError()
  ...
  if lcd.drawTextLines then
    lcd.drawTextLines(12, 12, widget.zone.w - 24, widget.zone.h - 24, text, colors.primary2)
  else
    lcd.drawText(widget.zone.w / 2, widget.zone.h / 2, text, CENTER + VCENTER + SMLSIZE + colors.primary2)
  end
end
...
local function stepOff()
  if (LS_STEP ~= nil and stepSwitchPrevious ~= nil) then
    setStickySwitch(LS_STEP, stepSwitchPrevious)
    stepSwitchPrevious = nil
  end
  if (GV_ADJUST ~= nil and adjustPrevious ~= nil) then
...
local function stepOn()
  if (LS_STEP ~= nil and stepSwitchPrevious == nil and not getStickySwitch(LS_STEP)) then
    stepSwitchPrevious = false
    setStickySwitch(LS_STEP, true)
  end
  if (GV_ADJUST ~= nil and adjustPrevious == nil) then
    local current = model.getGlobalVariable(GV_ADJUST, 0)
    if current ~= 1 then
      adjustPrevious = current
      model.setGlobalVariable(GV_ADJUST, 0, 1)
    end
  end
end
...
local function GetCurve(crvIndex)
  ...
  if not tbl or type(tbl.y) ~= "table" then
    stepOff()
    setError("Missing curve CV" .. crvIndex + 1)
    return nil
  end

  if #tbl.y ~= N then
    stepOff()
    setError("Wrong number of points on curve CV" .. crvIndex + 1)
    return nil
  end
...
local function GetOutput(crvIndex)
  ...
  stepOff()
  setError("No output channel with curve CV" .. crvIndex + 1)
  return nil, nil
end -- GetOutput()
...
beginEditing = function()
  if not crvTbls[1] or not crvTbls[1].y then
    init()
  end
  if errMsg then
    return
